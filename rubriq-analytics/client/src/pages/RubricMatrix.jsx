...
import { Plus, Edit2, Trash2, ChevronDown, ChevronRight, Download, Eye } from 'lucide-react';
...
export default function RubricMatrix() {
  ...
  const load = async () => {
    ...
    setRubrics(res.data);
    if (res.data.length > 0 && !selectedRubric) {
      const full = await rubricsAPI.get(res.data[0].id);
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedRubric && (
        <>
          {/* Rubric Header */}
          <div className="card p-5 bg-gradient-to-r from-navy-800 to-navy-600 text-white" style={{ background: 'linear-gradient(135deg, #1e3a5f, #2563eb)' }}>
          ...
          <div className="card p-4">
            ...
            <div className="grid grid-cols-4 gap-3">
              {[
                { level: 'Excellent', range: '24–30', color: '#16a34a', bg: '#f0fdf4', border: '#bbf7d0' },
                { level: 'Good', range: '18–23', color: '#2563eb', bg: '#eff6ff', border: '#bfdbfe' },
                { level: 'Satisfactory', range: '12–17', color: '#f59e0b', bg: '#fffbeb', border: '#fde68a' },
                { level: 'Needs Improvement', range: '0–11', color: '#ef4444', bg: '#fef2f2', border: '#fecaca' },
              ].map(l => (
                <div key={l.level} className="rounded-lg px-3 py-2.5 border" style={{ background: l.bg, borderColor: l.border }}>
                  ...
                  <p className="text-xs mt-0.5" style={{ color: l.color }}>{l.range} marks</p>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Criterion Modal */}
      <Modal open={modal.open && modal.type === 'criterion'} onClose={() => setModal({ open: false, type: '', data: null, parentId: null })}
        title={modal.data ? 'Edit Criterion' : 'Add Criterion'}
        footer={<><button className="btn-secondary" onClick={() => setModal({ open: false, type: '', data: null, parentId: null })}>Cancel</button><button className="btn-primary" onClick={handleSave} disabled={saving}>{saving ? 'Saving...' : 'Save'}</button></>}>
      ...
      {/* Level Modal */}
      <Modal open={modal.open && modal.type === 'level'} onClose={() => setModal({ open: false, type: '', data: null, parentId: null })}
